function selections = rBCS(classifiers, data, params)
...
while sum(discarded)>0
    ...
    %         disp(sprintf('Prev Ensemble: %2.5f', ensembleAcc_1));
    %         disp(sprintf('Next Ensemble: %2.5f', ensembleAcc_2));
...
% disp(sprintf('Original classifiers length: %2.5f', length(classifiers)));
% disp(sprintf('After BCS classifiers length: %2.5f', sum(selections)));
end
